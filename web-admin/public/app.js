...
function switchModal(render){const dialog=$('#modal');if(!dialog.open){render();return;}dialog.addEventListener('close',()=>requestAnimationFrame(render),{once:true});dialog.close();}
...
function openWorkerActions(worker,openSession,weekSessions,monthSessions){
  ...
  $('#worker-edit').onclick=()=>switchModal(()=>openWorker(worker));
  ...
  $$('.edit-recent-session').forEach(button=>button.onclick=()=>{const session=recent.find(s=>String(s.id)===button.dataset.id);if(!session){toast('Unable to open this time entry. Refresh and try again.',true);return;}switchModal(()=>openSession(session,renderDashboard));});
}
