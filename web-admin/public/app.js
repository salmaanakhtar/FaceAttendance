...
async function renderDashboard(){
  const today=isoDate(); const [now,stats,employees,attendance]=await Promise.all([
    api('/api/v1/admin/attendance/now'),api(`/api/v1/admin/attendance/stats?from=${today}&to=${today}`),
    api('/api/v1/admin/employees?status=active&limit=200&offset=0'),api(`/api/v1/admin/attendance?from=${today}&to=${today}&limit=200`)
  ]);
  state.cache.employees=employees.employees||[]; const agg=stats.aggregate||{}; const current=now.currentlyIn||[]; const sessions=attendance.sessions||[];
  $('#page').innerHTML=`
    <div class="stats"><div class="stat"><span>Workers currently in</span><strong class="positive">${current.length}</strong></div><div class="stat"><span>Today's sessions</span><strong>${agg.sessions??sessions.length}</strong></div><div class="stat"><span>Late today</span><strong class="warning">${agg.lateCount??0}</strong></div><div class="stat"><span>Incomplete</span><strong class="negative">${agg.incompleteCount??0}</strong></div></div>
    ...
    <section class="card"><div class="card-head"><h3>Today</h3>${pill(`${state.cache.employees.length} active workers`)}</div>${sessions.length?`<div class="table-wrap"><table><thead><tr><th>Worker</th><th>In</th><th>Out</th><th>Hours</th></tr></thead><tbody>${sessions.slice(0,12).map(s=>`<tr><td>${esc(s.employeeName)}</td><td>${esc(fmtTime(s.checkInAt))}</td><td>${esc(fmtTime(s.checkOutAt))}</td><td>${esc(hours(s.workedMinutes))}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">No attendance recorded today.</div>'}</section></div>`;
}
...
function openSession(session,onSaved){
  modal('Edit attendance',`<form id="session-form" class="form-grid"><div class="wide note">${esc(session.employeeName)} · ${esc(session.workDate)}</div><label>Clock in<input name="checkIn" type="datetime-local" required value="${esc(localInput(session.checkInAt))}"></label><label>Clock out<input name="checkOut" type="datetime-local" value="${esc(localInput(session.checkOutAt))}"></label><label class="wide">Reason<input name="reason" value="Administrator correction" required></label><p id="session-error" class="form-error wide" hidden></p><div class="modal-actions wide"><button type="button" class="secondary" id="cancel-session">Cancel</button><button class="primary">Save changes</button></div></form>`);$('#cancel-session').onclick=()=>$('#modal').close();$('#session-form').onsubmit=async(event)=>{event.preventDefault();const f=new FormData(event.currentTarget);try{const changes=[];const checkIn=new Date(f.get('checkIn')).toISOString();const out=f.get('checkOut');if(checkIn!==new Date(session.checkInAt).toISOString())changes.push(['check_in',checkIn]);if(out&&(!session.checkOutAt||new Date(out).toISOString()!==new Date(session.checkOutAt).toISOString()))changes.push(['check_out',new Date(out).toISOString()]);for(const [field,value] of changes)await api('/api/v1/admin/corrections',{method:'POST',body:{employeeId:session.employeeId,sessionId:session.id,field,value,reason:f.get('reason').trim()}});$('#modal').close();toast(changes.length?'Attendance updated.':'No changes to save.');await onSaved();}catch(err){const e=$('#session-error');e.textContent=err.message;e.hidden=false;}};
}
